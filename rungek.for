...
!##########################################################################
        ...
        use multidata
        implicit none
        double precision :: X_R_e,X_R_w,gamma,u_current
        integer :: i,j,k,ib
        integer :: is,ie,js,je,ks,ke
        double precision :: dxx,dyy,dzz,visc,diff
...
!===========Relaxation zone==================
          ...
          if ((L_beach).and.(beach.eq.2)) then
       
           X_R_e = (dom(ib)%x(i)-(x_end_beach-(2.0d0*L_w)))/(2.0d0*L_w)
           gamma = 1.0d0-(exp(X_R_e**3.5)-1.0)/(exp(1.0)-1.0)
           !
           if(abs(grz).gt.0.1)then
            u_current = ubulk ! velocity profile of the channel, parabolic??
           !u_current=ubulk*(0.0063+0.5643*dom(ib)%zc(k)-
      !& 1.2418*dom(ib)%zc(k)**2.0+3.1141*dom(ib)%zc(k)**3.0-
      !& 2.5746*dom(ib)%zc(k)**4.0)
           elseif(abs(gry).gt.0.0)then
            u_current = ubulk ! velocity profile of the channel, parabolic??
           !u_current=ubulk*(0.0063+0.5643*dom(ib)%yc(j)-
      !& 1.2418*dom(ib)%yc(j)**2.0+3.1141*dom(ib)%yc(j)**3.0-
      !& 2.5746*dom(ib)%yc(j)**4.0) ! velocity profile at the boundary layer on the bottom
           endif
            !
            if (X_R_e.ge.0.0d0) then
             dom(ib)%ustar(i,j,k)=dom(ib)%ustar(i,j,k)*gamma+
     &(1-gamma)*u_current
...
!===========================================================
        ...
        X_R_e = (dom(ib)%xc(i)-(x_end_beach-(2.0d0*L_w)))/(2.0d0*L_w)  
	gamma = (exp(X_R_e**2.0)-1.0)/(exp(1.0)-1.0)
	    if (X_R_e.lt.0.0d0) then
	     dom(ib)%vstar(i,j,k)=dom(ib)%vstar(i,j,k)+dt*alfapr*gry*
     & cos(atan(slope))
            else if (X_R_e.ge.0.0d0) then
	     dom(ib)%vstar(i,j,k)=dom(ib)%vstar(i,j,k)+dt*alfapr*gry*
     & cos(atan(slope))-dt*alfapr*(20.0d0+0.0d0*
     & abs(dom(ib)%vstar(i,j,k)))*gamma*dom(ib)%vstar(i,j,k)
            end if
...
!===========================================================
        ...
        gamma = 1.0d0-(exp(X_R_e**3.5)-1.0)/(exp(1.0)-1.0)
          ...
          if (X_R_e.ge.0.0d0) then
           dom(ib)%vstar(i,j,k)=dom(ib)%vstar(i,j,k)*gamma
          end if
...
!===========================================================
	...
	gamma = (exp(X_R_e**2.0)-1.0)/(exp(1.0)-1.0)
     ...
     & cos(atan(slope))
	     ...
	     dom(ib)%wstar(i,j,k)=dom(ib)%wstar(i,j,k)+dt*alfapr*grz*
     & cos(atan(slope))-dt*alfapr*(20.0d0+0.0d0*
     & abs(dom(ib)%wstar(i,j,k)))*gamma*dom(ib)%wstar(i,j,k)
            end if
...
!===========================================================
        ...
        gamma = 1.0d0-(exp(X_R_e**3.5)-1.0)/(exp(1.0)-1.0)
          ...
          if (X_R_e.ge.0.0d0) then
           dom(ib)%wstar(i,j,k)=dom(ib)%wstar(i,j,k)*gamma
          end if

...
!##############################################################################
      ...
      double precision :: n_r,U_corr,k_w,c,dn,ddn
      double precision :: dddn,n_h,ep,omega,u_prof,z_stretch
...
!===== Aristos 25/02/19 Waves ==========================================
           elseif ( (dom(ib)%bc_west.eq.31) .and.
     &      (abs(grz).gt.0.0001d000) ) then
            u_prof = 0.0d00
              do k=ks-1,ke+1; do j=js-1,je+1
                if (L_LSM) then
                  
                 if(ly.eq.0)then
                  if(k.eq.abs(ks-1))then
                     if(j.eq.abs(js-1))then
                     !write(*,*) "boundu value grz ", grz
                     endif
                  endif
                 endif

                if (theory.eq.1) then                                   !Linear Theory
                 k_w=2.0d0*PI/L_w
                 omega = sqrt(9.81d0*k_w*tanh(k_w*length))              !======== k_w: wavenumber,omega:freq
                 
		 if (dom(ib)%phi(is,j,k) .ge. 0.d0) then                  
                  dom(ib)%u(is-1-ly,j,k)= (H_w/2.0d0)*omega*
     & (cosh(k_w*dom(ib)%zc(k))/sinh(k_w*length))*cos(omega
     & *ctime-PI/2.0d0)+u_prof
                  else
                ...
                z_stretch=(dom(ib)%zc(k)/(length+H_w/2.0d0*         !new coordinate to maintain velocity profile
     & cos(omega*ctime-PI/2.0d0)+(H_w**2.0*k_w/16.0d0)*
     & (3.0d0/((tanh(k_w*length))**3.0)-1.0d0/(tanh(k_w*length)))
     & *cos(2.0d0*(omega*ctime-PI/2.0d0))))*length
               !
               u_prof=ubulk
               !u_prof=ubulk *(0.0063+0.5643*z_stretch-                  !Eugeny paper Current profile added to u_vel
      !& 1.2418*z_stretch**2.0+3.1141*z_stretch**3.0-
      !& 2.5746*z_stretch**4.0) 

                  if (dom(ib)%phi(is,j,k) .ge. 0.0d0) then             
                  dom(ib)%u(is-1-ly,j,k)= (H_w/2.0d0)*omega*
     & (cosh(k_w*dom(ib)%zc(k))/sinh(k_w*length))*cos(omega
     & *ctime-PI/2.0d0)+(3.0d0/16.0d0)*k_w*omega*H_w**2.0*
     & (cosh(2.0d0*k_w*dom(ib)%zc(k))/(sinh(k_w*length))**4.0)*cos(2.d0
     & *(omega*ctime-PI/2.0d0))+u_prof
                  else
                  dom(ib)%u(is-1-ly,j,k)= 0.d0
                  end if
                  !
                elseif (theory.eq.3) then                               !Solitary Wave
		 k_w=sqrt(3.0d0*H_w/(4.0d0*length**3.0))                
                 c=sqrt(9.81*(H_w+length)); ep=H_w/length               
     ...
     &      (abs(gry).gt.0.0001d000) ) then
		 ...
		 if (dom(ib)%phi(is,j,k) .ge. 0.d0) then                  
                  dom(ib)%u(is-1-ly,j,k)= (H_w/2.0d0)*omega*
     & (cosh(k_w*dom(ib)%yc(j))/sinh(k_w*length))*cos(omega
     & *ctime-PI/2.0d0)+u_prof
                  else
                  dom(ib)%u(is-1-ly,j,k)= 0.d0
                 end if
                ...
                z_stretch=(dom(ib)%yc(j)/(length+H_w/2.0d0*         !new coordinate to maintain velocity profile
     & cos(omega*ctime-PI/2.0d0)+(H_w**2.0*k_w/16.0d0)*
     & (3.0d0/((tanh(k_w*length))**3.0)-1.0d0/(tanh(k_w*length)))
     & *cos(2.0d0*(omega*ctime-PI/2.0d0))))*length
               u_prof=ubulk
     ...
     & (cosh(k_w*dom(ib)%yc(j))/sinh(k_w*length))*cos(omega
     & *ctime-PI/2.0d0)+(3.0d0/16.0d0)*k_w*omega*H_w**2.0*
     & (cosh(2.0d0*k_w*dom(ib)%yc(j))/(sinh(k_w*length))**4.0)*cos(2.d0
     & *(omega*ctime-PI/2.0d0))+u_prof
                  else
                  dom(ib)%u(is-1-ly,j,k)= 0.d0
                  end if
     ...
     & *tanh(tau)/(cosh(tau))**4.0
                  ...
                  dom(ib)%u(is-1-ly,j,k)= sqrt(9.81*length)*ep*
     & (n_h-(1.0d0/4.0d0)*ep*n_h**2.0+(length**2.0d0/(3.0d0*
     & c**2.0))*(1.0d0-(3.0d0*dom(ib)%yc(j)**2.0/(2.0*
     & length**2.0)))*ddn)+u_prof
                  else
                  dom(ib)%u(is-1-ly,j,k)= 0.d0
                  end if
...
!...............................................................................
     ...
     & dom(ib)%uoo(ie,j,k))/dom(ib)%dx
           ...
           end if ! dom(ib)%bc_east flag
        end if ! dom(ib)%inext index
!...............................................................................
	  ...
	  !pablo:	03/19
        ...
        if (dom(ib)%knext.lt.0) then
           if (dom(ib)%bc_top.eq.4) then
             ...
             !
      else if (dom(ib)%bc_top.eq.2 .or.dom(ib)%bc_top.eq.21) then
          if (alfabc.eq.1)then
            do j=js-1,je+1; do i=is-1,ie+1
               ! 
            if (L_LSM) then
               !
                if (dom(ib)%bc_top.eq.2) then
                   dom(ib)%u(i,j,ke+1+ly)=dom(ib)%u(i,j,ke)
                else
                   dom(ib)%u(i,j,ke+1+ly)=dom(ib)%uoo(i,j,ke+1+ly) 
     & -dt*alfapr*(dom(ib)%uoo(i,j,ke+1+ly)-
     & dom(ib)%uoo(i,j,ke))/dom(ib)%dz
               end if
               !
       else		!no LSM
          ! 
...
!===== Aristos 25/02/19 Waves ==========================================
              ...
              do k=ks-1,ke+1; do j=js-1,je+1
                ...
                if (theory.eq.1) then                                           !Linear Theory
                 ...
                 omega = sqrt(9.81d0*k_w*tanh(k_w*length))                      !======== k_w: wavenumber,omega:freq
                  ...
                  if (dom(ib)%phi(is,j,k) .ge. 0.d0) then                     
                  dom(ib)%v(is-1-ly,j,k)= -(H_w/2.0d0)*omega*
     & (sinh(k_w*dom(ib)%y(j))/sinh(k_w*length))*sin(omega
     & *ctime-PI/2.0d0)
                 else
                  dom(ib)%v(is-1-ly,j,k)= 0.d0
                  end if
     ...
     & (sinh(k_w*dom(ib)%y(j))/sinh(k_w*length))*sin(omega
     & *ctime-PI/2.0d0)-(3.0d0/16.0d0)*k_w*omega*H_w**2.0*
     & (sinh(2.0d0*k_w*dom(ib)%y(j))/(sinh(k_w*length))**4.0)*sin(2.d0
     & *(omega*ctime-PI/2.0d0))
                  else
                  dom(ib)%v(is-1-ly,j,k)= 0.d0
                  end if
     ...
     & *tanh(tau)/(cosh(tau))**4.0
                  ...
                  if (dom(ib)%phi(is,j,k) .ge. 0.d0) then
                  dom(ib)%v(is-1-ly,j,k)= dom(ib)%y(j)/c*
     & sqrt(9.81*length)*ep*((1.0d0-0.5d0*ep*n_h)*dn+
     & (length**2.0/(3.0d0*c**2.0))*(1.0d0-(dom(ib)%yc(j)**2.0
     & /(2.0d0*length**2.0)))*dddn)

!                  dom(ib)%v(is-1-ly,j,k)=(dom(ib)%y(j)/(length+H_w
!     & /cosh(k_w*(-c*ctime)+tau)**2.0))*(2.0*H_w/cosh(k_w*(-c*ctime)
!     & +tau)**2.0)*tanh(k_w*(-c*ctime)+tau)*(c-dom(ib)%u(is-1-ly,j,k))
                  else
                  dom(ib)%v(is-1-ly,j,k)= 0.d0
                  end if
...
!#############################################################################
      ...
      real, parameter :: PI = 4 * atan (1.0)	
      double precision :: k_w,c,omega,dn,ddn
      double precision :: dddn,n_h,ep
...
!===== Aristos 25/02/19 Waves ==========================================
              ...
              do k=ks-1,ke+1; do j=js-1,je+1
                ...
                if (theory.eq.1) then                                           !Linear Theory
                 ...
                 omega = sqrt(9.81d0*k_w*tanh(k_w*length))                      !======== k_w: wavenumber,omega:freq
                  ...
                  if (dom(ib)%phi(is,j,k) .ge. 0.d0) then                     
                  dom(ib)%w(is-1-ly,j,k)= -(H_w/2.0d0)*omega*
     & (sinh(k_w*dom(ib)%z(k))/sinh(k_w*length))*sin(omega
     & *ctime-PI/2.0d0)
                 else
     ...
     & (sinh(k_w*dom(ib)%z(k))/sinh(k_w*length))*sin(omega
     & *ctime-PI/2.0d0)-(3.0d0/16.0d0)*k_w*omega*H_w**2.0*
     & (sinh(2.0d0*k_w*dom(ib)%z(k))/(sinh(k_w*length))**4.0)*sin(2.d0
     & *(omega*ctime-PI/2.0d0))
                  else
                  dom(ib)%w(is-1-ly,j,k)= 0.d0
                  end if
...
!                  dom(ib)%w(is-1-ly,j,k)=(dom(ib)%z(k)/(length+H_w
!     & /cosh(k_w*(-c*ctime)+tau)**2.0))*(2.0*H_w/cosh(k_w*(-c*ctime)
!     & +tau)**2.0)*tanh(k_w*(-c*ctime)+tau)*(c-dom(ib)%u(is-1-ly,j,k))
                  else
                  dom(ib)%w(is-1-ly,j,k)= 0.d0
...
!	       dom(ib)%w(i,j,ktop+1) = 0.0				!Pablo 03/19
     ...
     &				(dom(ib)%bc_top.ge.61)) then
         ...
         if (L_LSM) then
         !
         !if (dom(ib)%phi(i,j,ke) .ge. 0.0) then
         if (dom(ib)%bc_top.eq.2) then
         dom(ib)%w(i,j,ke+1+ly)= dom(ib)%w(i,j,ke)
         else
         dom(ib)%w(i,j,ke+1+ly)= dom(ib)%woo(i,j,ke+1+ly)
     & -dt*alfapr*(dom(ib)%woo(i,j,ke+1+ly)-
     & dom(ib)%woo(i,j,ke))/dom(ib)%dz
         end if ! dom(ib)%bc_top
         !else
         !dom(ib)%w(i,j,ke+1+ly)= dom(ib)%w(i,j,ke) 
         !endif ! dom(ib)%phi
         !
     ...
     & dom(ib)%woo(i,j,ke))/dom(ib)%dz
      ...
      endif ! dom(ib)%bc_top.eq.2 .or.dom(ib)%bc_top.eq.21
           !
	  	  if (trim(keyword).eq.'cavity') then	
		  do j=js-1,je+1 ; do i=is-1,ie+1
                 dom(ib)%w(i,j,ke+1+ly)=0.0	

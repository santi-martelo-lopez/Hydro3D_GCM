...
!######################################################################
	SAVE
      DOUBLE PRECISION :: X_R_e,X_R_w,h,gamma
...
! xc -> module_multidata.for/line 41
!========== Richard cube test =========================================
...
!========== dambreak test (Dimitrios Pavlidi 2016) =========================================
          if (dom(ib)%xc(i).ge.100) then
            if(abs(grz).gt.0.0001d00)then
            dom(ib)%phi(i,j,k) = -1.d0*dabs(dom(ib)%zc(k)-0.d0)
            elseif(abs(gry).gt.0.0001d00)then
            dom(ib)%phi(i,j,k) = -1.d0*dabs(dom(ib)%yc(j)-0.d0)
                  ...
                  if (dom(ib)%yc(j).lt.length)   then
                   dom(ib)%phi(i,j,k) = 1.d0*dabs(dom(ib)%yc(j)-length)
...
!
          ...
          !
          elseif (trim(keyword).eq.'waves') then      ! Channel flow case
            !do k=1,ttk-4                
            !  do j=1,ttj-4    
            !    do i=1,tti-4
            do k=1,ttk                
      ...
      !
		...
		b = (0.7d0/(cosh(sqrt(3.0d0*0.7d0)/2.0d0*
     & dom(ib)%xc(i)))**2.0d0)+length
                  if(abs(grz).gt.0.0001d00)then
                  if (dom(ib)%zc(k).lt.b)   then
                    dom(ib)%phi(i,j,k) = 1.0*abs(dom(ib)%zc(k)-b)
                  else if (dom(ib)%zc(k).gt.b)   then
                    dom(ib)%phi(i,j,k) = -1.0*abs(dom(ib)%zc(k)-b)
                  else if (dom(ib)%zc(k).eq.b)  then
                    dom(ib)%phi(i,j,k) = 0.0
                  endif
                  elseif(abs(gry).gt.0.0001d00)then
                  if (dom(ib)%yc(j).lt.b)   then
                    dom(ib)%phi(i,j,k) = 1.0*abs(dom(ib)%yc(j)-b)
                  else if (dom(ib)%yc(j).gt.b)   then
                    dom(ib)%phi(i,j,k) = -1.0*abs(dom(ib)%yc(j)-b)
                  else if (dom(ib)%yc(j).eq.b)  then
                    dom(ib)%phi(i,j,k) = 0.0
                  endif
                  endif
                  dom(ib)%s_phi0(i,j,k)=dom(ib)%phi(i,j,k)
...
!===========================================================================================
! Relaxation zone near East Boundary ===== ====
!===========================================================================================
! for z=length phi_target = 0
! for z<length phi_target = 1.d0*dabs(dom(ib)%zc(k)-length)
! for z>length phi_target = -1.d0*dabs(dom(ib)%zc(k)-length)

      if ((L_beach).and.(beach.eq.1)) then
                    
	X_R_e = (dom(ib)%xc(i)-(x_end_beach-(L_w/2.0d0)))/(L_w/2.0d0)           !== Aristos Christou == X_R = apply relaxation at the very end (L/2)  
           if (X_R_e.ge.0) then
           if(abs(grz).ge.0.0001d0)then
     ...
     &            (1.0-gamma)*(-1.d0*abs(dom(ib)%zc(k)-length))
           ...
           elseif(abs(gry).ge.0.0001d0)then
	    gamma = 1.0-((exp(X_R_e**3.5)-1.0)/(exp(1.0)-1.0))

                if (dom(ib)%yc(j).eq.length) then
                  dom(ib)%phi(i,j,k) = dom(ib)%phi(i,j,k)
     &  	  *gamma
                else if (dom(ib)%yc(j).lt.length)   then
                   dom(ib)%phi(i,j,k) = gamma*dom(ib)%phi(i,j,k)+
     &            (1.0-gamma)*(1.d0*abs(dom(ib)%yc(j)-length))
                else if (dom(ib)%yc(j).gt.length)   then
                   dom(ib)%phi(i,j,k) = gamma*dom(ib)%phi(i,j,k)+
     &            (1.0-gamma)*(-1.d0*abs(dom(ib)%yc(j)-length))
                end if
        ...
        else if ((L_beach).and.(beach.eq.2)) then
       !
         X_R_e = (dom(ib)%xc(i)-(x_end_beach-(2.0d0*L_w)))/(2.0d0*L_w)         !== Aristos Christou == X_R = apply relaxation at the very end (2L)
            if (X_R_e.ge.0) then
           if(abs(grz).ge.0.0001d0)then
            gamma = 1.0-((exp(X_R_e**3.5)-1.0)/(exp(1.0)-1.0))
                if (dom(ib)%zc(k).eq.length) then
...
!
           if (trim(keyword).eq.'channel' .and. lends) then
             ...
             !!!    elseif(abs(gry).gt.0.0001d00)then
             !!!    if (dom(ib)%yc(j).lt.length)   then
             !!!      dom(ib)%phi(i,j,k) = 1.0*abs(dom(ib)%yc(j)-length)
             !!!    else if (dom(ib)%yc(j).gt.length)   then
             !!!      dom(ib)%phi(i,j,k) = -1.0*abs(dom(ib)%yc(j)-length)
             !!!    else if (dom(ib)%yc(j).eq.length)  then
             !!!      dom(ib)%phi(i,j,k) = 0.0
             !!!    endif
             !!!    endif ! gry, grz values
             !!!    endif ! Distance phi fixed    
             !!!end if ! dom(ib)%iprev
            !
            !
            ! Hold level to be held constant at outflow (if required - may help with stability in inflow-outflow sims)
           ...
           !elseif (trim(keyword).eq.'waves' .and. lends) then
           !  if (dom(ib)%iprev.lt.0) then
           !    if ((i.ge.dom(ib)%isu).and.(i.le.dom(ib)%isu+5)) then   ! Distance phi fixed                 
           !      if(abs(grz).gt.0.0001d00)then
           !      if (dom(ib)%zc(k).lt.length)   then
           ...
           !      endif ! Distance phi fixed    
           !  end if ! dom(ib)%inext
           !elseif (trim(keyword).eq.'wave' .and. lends) then
           !  if (dom(ib)%iprev.lt.0) then
           !    if ((i.ge.dom(ib)%isu).and.(i.le.dom(ib)%isu+5)) then   ! Distance phi fixed                 
           !      if(abs(grz).gt.0.0001d00)then
           !      if (dom(ib)%zc(k).lt.length)   then
...
!
      n_epsl = 1.00001d00	!n_epsl = 1.9999999d0           ! Transition zone across free surface (2 grid cells width either side)

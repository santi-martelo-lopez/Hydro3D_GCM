...
!================== Start Time Loop ====================================
        do itime=itime_start,itime_end
         ...
         !
         !
         !!!if(LRESTART.eq..TRUE.)then
            if(itime+1.lt.abs(itime_start+tr_iter))then
            !
            !maxcy rump function
            if(maxcy_rf.eq.0)then ! step function
            maxcy = maxcy_bc*maxcy_factor
            elseif(maxcy_rf.eq.1)then ! linear function
            rampFactor = int((0-maxcy_factor)*
     &  (itime-abs(itime_start+tr_iter))/tr_iter)
            maxcy = maxcy_bc*max(1,rampFactor) ! in case rampfactor is 0
            endif ! maxcy_rf flag
            !
            if (L_LSM)  then
            if(lsm_rf.eq.0)then ! step function
            ntime_reinit = lsm_bc*lsm_factor
            elseif(lsm_rf.eq.1)then ! linear function
            rampFactor = int((0-lsm_factor)*
     &  (itime-abs(itime_start+tr_iter))/tr_iter)
            ntime_reinit = lsm_bc*max(1,rampFactor) ! in case rampfactor is 0
            endif ! lsm_rf flag
            endif ! L_LSM flag
            !
            !dt rump function
            if(dt_rf.eq.0)then ! step function
            dt = dt_bc/dt_factor
            elseif(dt_rf.eq.1)then ! linear function
      !       dt = dt_bc / (100*int(100/int((0-dt_factor)*(
      !&  itime-abs(itime_start+tr_iter))/tr_iter)))
            rampFactor = int((0-dt_factor)*(
     &  itime-abs(itime_start+tr_iter))/tr_iter)
            dt = dt_bc / max(1,rampFactor) ! in case rampfactor is 0
            endif ! dt_rf flag
            !
            !
            !
            else ! using default values specified in control.cin
            maxcy = maxcy_bc
            ntime_reinit = lsm_bc
            dt = dt_bc
            !
         ...
         !
         !
         !! Lets apply a ramp function for the inlet velocity
         !if(abs(ubulk_bc).gt.0.1)then ! threshold check
         !   if(itime.lt.uTrans)then
         !      ubulk = ubulk_bc*(lowerU+upperU*(itime/uTrans))
         !   else
         !      ubulk = ubulk_bc
         !   endif ! transition iter check
         !   write(6,*) "Ubulk = ", ubulk
         !endif ! UBULK threshold check
         !
         !
...
!-----------------------------------------------------------------------
           ...
           do ib=1,nbp
              ...
              do j=1,dom(ib)%ttc_j
      !    ! Lets apply a ramp function for the inlet velocity
      !    if(abs(ubulk.gt.0.1 ))then ! threshold check
      !       if(itime.lt.uTrans)then
      !          dom(ib)%uoo(i,j,k)=dom(ib)%u(i,j,k) + ubulk_bc
      !       else
...
!Initial calculations in IBM to allocate particles to domains and cpus
           ...
           if (LIMB)  then
            if(motionFlag.eq.1) then
               if (myrank.eq.0) write(*,*) "Searching for ghost cells" 
               ...
               call checkEmergingCells ! it checks if mucx = mucxold ?
                  ...
                  ! mucxold = mucx
                  ! mucx = 0
               !!!call ghostCell(L_LSM,0) ! updating solid cells after the movement of the solid
               ...
               if (itime.eq.itime_start) then
                  ...
                  if (myrank.eq.0) write(*,*) "Finished searching ", 
     & "for ghost cells"
               endif
               !
               !!!call ghostCell(L_LSM,0) ! updating solid cells after the movement of the solid
           ...
           !
            !if ((mod(itime,n_out).eq.0).and.(itime.eq.itime_start)) then
            if ((mod(itime,n_out).eq.0).and.(itime.ge.itime_start)) then
               if (n_unstpt .gt. 0) then
               do i=1,n_unstpt
            call save2vtk_rectilinearPlane(itime,id_unst(i),j_unst(i))
               enddo
               endif
               ...
               if (L_LSM)  then
               call save2vtk_rectilinearGrid(itime,7)  ! 7 : flag for writting phi values
               call save2vtk_rectilinearGrid(itime,8)  ! 8 : flag for writting density values
               endif
            endif
           !
!Release and allocation of particles in LPT
	     if (LPT) then
	...
	if (myrank.eq.0) wtime_cd = MPI_WTIME ( )
           if(conv_sch.eq.3 .or. conv_sch.eq.4) then
            do kutta=1,kuttacond
               ...
               !
! Applying the ghost cell method for velocities
               !!!call ghostCell(L_LSM,1)
               !
...
! Applying the ghost cell method after solving the Pressure Poisson Equation
        if (myrank.eq.0) write(*,*) "Pressure terms solved, "
     &  "applying ghost cell method" 
        call ghostCell(L_LSM,2)
        if ((mod(itime,10).eq.0).and.(itime.ge.itime_start)) then
         call ghostCell_forces
        endif

...
! strictly bigger thant variable maxc = INT(R(M)/dxm) + 1   
	...
	! Total number of layers possible to define
		...
		allocate(ibmSt(bodynum))	! CHECK, this may not be a good place to do the allocation, it is better when reading the
									! number of bodies
		ibmSt(M) = nlay
		if(M.eq.1) then
			imbSt_k =  nlay
		else
			imbSt_k = imbSt_k + nlay
		endif
		write(*,*) "Number of Stations: ",imbSt_k

		allocate(ibmStMkrs(imbSt_k))	! CHECK, this may not be a good place to do the allocation
		allocate(ibmMkrsExtLy(imbSt_k))	! CHECK, this may not be a good place to do the allocation
	 ...
	 nodesPerStation = 0
		...
		!!!!!!!!!!!!!!!!!!!!!
		! Lets initialize the global variable keeping track on which station is a certain markers located
		allocate(ibmStation(nodes(M)))
		 ...
		 do K = 1, nlay
			...
			ibmMkrsExtLy( (M-1)*ibmSt(M-1) + M*k ) = nodes_percyl(1)
		 enddo
			write(*,*) "Number of markers in the exterior layer: ",ibmMkrsExtLy(1)
		 ...
		 allocate(MrkrsSection (bodynum,NumStation(M)))		  
			...
			ibmStMkrs( (M-1)*ibmSt(M-1) + M*k ) = nodesPerStation
		 enddo
		  write(*,*) "Number of markers per station: ",ibmStMkrs(1)
...
!#############################################################
      SUBROUTINE imb_oscillating_cylinder(numIB)
!#############################################################
      use vars
      use multidata
      use imb
	...
	disz = amp_z*DSIN( frec_z*(CTIME-startTime*dt) )	! ! vertical displacement in oscillating cylinder case


	! Updating node velocities

	U_p = 0.0d0
	W_p = frec_z*amp_z*DCOS(frec_z*(CTIME-startTime*dt) )
	V_p = 0.0d0

	if(itime .lt. (startTime+2) ) then
        do L = 1, nodes(numIB)
